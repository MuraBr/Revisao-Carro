...
async function submeter() {
  ...
  try {
    // Note que passamos o form.id em AMBAS as verificações
    const [resCpf, resEmail] = await Promise.all([
      axios.get('/api/clientes/verificar-cpf', { 
        params: { cpf: form.cpf, id: form.id } 
      }),
      axios.get('/api/clientes/verificar-email', { 
        params: { email: form.email, id: form.id } 
      })
    ])

    if (resCpf.data.existe) {
      erros.cpf = 'Este CPF já está sendo usado por outro cliente.'
    }
    if (resEmail.data.existe) {
      erros.email = 'Este email já está sendo usado por outro cliente.'
    }

    // Se houver qualquer erro de duplicidade, paramos aqui
