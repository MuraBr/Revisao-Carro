<template>
  <AppLayout :titulo="`Veículos de ${clienteNome}`">
    <div class="px-2">
      ...
      <div class="flex items-center gap-3 mb-6 pb-6">
        ...
        <input v-model="pesquisa" type="text" placeholder="Pesquisar por marca ou modelo..."
      ...
      <div v-if="mostraCadastrar"
        ...
        <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
          <h3 class="font-bold text-gray-800 text-lg mb-4">Novo Veículo</h3>
          <CadastrarVeiculo @salvar="criarVeiculo" @cancelar="mostraCadastrar = false" />
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-6">
          <div v-for="v in veiculoStore.veiculos" :key="v.id"
            ...
            <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
              <RouterLink :to="`/clientes/${clienteId}/veiculos/${v.id}/revisoes?nome=${clienteNome}`"
...
import CadastrarVeiculo from '../components/forms/cadastrar/CadastrarVeiculo.vue'
...
function mudarPagina(novaPagina) {
  veiculoStore.buscarDoCliente(clienteId, novaPagina, pesquisa.value)
  window.scrollTo({ top: 0, behavior: 'smooth' })
}

async function criarVeiculo(form) {
  await veiculoStore.criar(clienteId, form)
  mostraCadastrar.value = false

  veiculoStore.buscarDoCliente(clienteId)
}
...
async function salvarModificacao(form) {
  await veiculoStore.atualizar(clienteId, veiculoSelecionado.value.id, form)
  mostraModificar.value = false
}

async function excluirVeiculo(id) {
  await veiculoStore.excluir(clienteId, id)
}
</script>